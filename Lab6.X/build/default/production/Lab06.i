...
isr: ; rutina de interrupción
    btfsc ((INTCON) and 07Fh), 2 ; Si está encendida la bandera, entonces
    call int_tmr0 ; va a la subrutina del TMR0
    btfsc ((PIR1) and 07Fh), 0 ; Si está encendida la bandera, entonces
    call int_tmr1 ; va a la subrutina del TMR1
    btfsc ((PIR1) and 07Fh), 1 ; Si está encendida la bandera, entonces
...
int_tmr2:
    ...
    bcf ((PIR1) and 07Fh), 1 ;Apaga la bandera de interrupción
    return
...
config_int_enable:; INTCON
    ...
    Banksel TRISA
    bsf ((PIE1) and 07Fh), 0 ; Se habilitan la interrupción del TMR1 y TMR2 Registro PIE1
    bsf ((PIE1) and 07Fh), 1
    Banksel PORTA
    bcf ((PIR1) and 07Fh), 0 ; Se limpian las banderas de interrupción Registro PIR1
    bcf ((PIR1) and 07Fh), 1
    return
...
config_tmr1:
    Banksel PORTA
    bsf ((T1CON) and 07Fh), 0 ; Se habilita el TMR1
    bcf ((T1CON) and 07Fh), 1 ; Seleccion del reloj interno
    bsf ((T1CON) and 07Fh), 4
    bsf ((T1CON) and 07Fh), 5 ; Prescaler a 1:8
    reiniciar_tmr1
    return

config_tmr2:
    banksel PORTA
    bsf ((T2CON) and 07Fh), 2 ; Se habilita el TMR2
    bsf ((T2CON) and 07Fh), 1
    bsf ((T2CON) and 07Fh), 0 ; Prescaler a 1:16
    bsf ((T2CON) and 07Fh), 6
    bsf ((T2CON) and 07Fh), 5
    bsf ((T2CON) and 07Fh), 4
    bsf ((T2CON) and 07Fh), 3 ;Postscaler 1:16
    bcf ((PIR1) and 07Fh), 1 ; Limpiar bit de interrupción por overflow (Bit de INTCON)
    Banksel TRISA
